...
void search_home(char ** conf_home){
	...
	if (home == NULL){
		...
		home_len_max = home_len + strlen(CONF_HOME_FILE);
		
		assert(home_len_max > home_len);
		assert(sizeof(char) == 1);
		assert(home_len_max < FILENAME_MAX );
		assert(home_len_max == (home_len + strlen(CONF_HOME_FILE)));

		(*conf_home) = malloc(home_len_max + 1);
		strncpy((*conf_home), home, home_len);
  ...
  if (try_read_local_configuration() == FALSE){
    ...
    if (try_read_home_configuration() == FALSE){
      ...
      if ( try_read_etc_configuration() == FALSE){
				...
				msg_queue_push("Can't read any configuration file", "");
				(void)fflush(stdout);
      }
    }
  }
